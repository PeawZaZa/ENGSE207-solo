...
app.get('/api/tasks/:id', (req, res) => {
    ...
    // db.get() is used to retrieve a single row
    db.get(sql, [id], (err, row) => {
        ...
        // If row is undefined, the task ID was not found
        if (!row) {
            // 404 Not Found
...
app.post('/api/tasks', (req, res) => {
    ...
    db.run(sql, [title, description, priority], function(err) {
        ...
        // 201 Created, return the ID of the new task
        res.status(201).json({ 
...
app.patch('/api/tasks/:id/status', (req, res) => {
    ...
    db.run(sql, [status, id], function(err) {
        ...
        if (this.changes === 0) {
            return res.status(404).json({ error: 'Task not found or status already set' });
        }
