...
async function createTask(taskData) {
    ...
    try {
        ...
        if (!response.ok) {
            // Wait for response body to get error message from server
            const errorData = await response.json();
            throw new Error(errorData.error || 'Failed to create task');
        }
        
        // Fetch tasks again to get the full task object with ID and created_at
        // A more efficient way would be to GET the new task by ID, but refetching all works for simplicity.
        await fetchTasks();
...
async function updateTaskStatus(taskId, newStatus) {
    ...
    try {
        ...
        // Update the task in the local state (allTasks)
...
async function deleteTask(taskId) {
    ...
    try {
        ...
        // Remove the task from the local state (allTasks)
...
7. Implement Part 5 (Update Status) <- COMPLETED
8. Implement Part 6 (Delete Task) <- COMPLETED
9. Add Part 12 (Event Listeners)
10. Add Part 13 (Initialization)
11. Add Part 14 (Global Functions) <- COMPLETED
